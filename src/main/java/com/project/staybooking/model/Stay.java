...
@Entity
@Table(name = "stay")
@JsonDeserialize(builder = Stay.Builder.class) //tell jackson library use builder pattern to new obj
public class Stay implements Serializable {
    private static final long serialVersionUID = 1L;

    @Id
    @GeneratedValue(strategy = GenerationType.AUTO)
    private Long id; //obj instead of primitive, if empty, ruturn null instead of 0
    private String name;
    private String description;
    private String address;
    @JsonProperty("guest_number") //naming convention
    private int guestNumber;
    @ManyToOne
    @JoinColumn(name = "user_id") //不要foreign key的中间表
    private User host;
    ...
    @JsonIgnore //otherwise the returned data will crash, because (date is an array of objects:id {sid,date}, stay{id, n, a, dates..}) -> infinite loop
    @OneToMany(mappedBy = "stay", cascade = CascadeType.ALL, fetch=FetchType.LAZY)
    private List<StayReservedDate> reservedDates;

    public Stay() {}

    private Stay(Builder builder) { //private property多时，排列组合比较多 2^n
        this.id = builder.id;
    ...
    public static class Builder { //nested static class
        ...
        @JsonProperty("id") //mapping private property to json key
        private Long id;
